...
static void delay_cycles(uint16_t cycles)
{
  while (cycles > 0)
  {
    __NOP();
...
void DD_timer_DMA_start(uint16_t ARR, uint16_t transfers)
{
  ...
  DMA2_Channel1->CNDTR = transfers/2;                            // number of transfers
  DMA2_Channel1->CPAR = (uint32_t)&(GPIOB->ODR);               // write to register TODO: get from object
  DMA2_Channel1->CMAR = (uint32_t)&DD_data_buffer[0];             // read from address
  // set mem size to 32 to use every second value and peripheral size to 16 bit, increment mem address, direction to peripheral, set very high priority // enable transfer complete interrupt
  DMA2_Channel1->CCR |= (DMA_CCR_MSIZE_1 | DMA_CCR_PSIZE_0 | DMA_CCR_MINC | DMA_CCR_DIR | DMA_CCR_PL_0 | DMA_CCR_PL_1); // | DMA_CCR_TCIE);

  // enable DMAMUX
  RCC->AHB1ENR |= RCC_AHB1ENR_DMAMUX1EN;
  // set up DMAMUX, DMA2_CH1 is linked to DMAMUX1_CH8, TIM8_CH1 is on MUX 49
  DMAMUX1_Channel8->CCR |= (STM32_DMAMUX1_TIM1_CH1); //  && DMAMUX_CxCR_DMAREQ_ID_Msk);

  // enable DMA channel
  DMA2_Channel1->CCR |= DMA_CCR_EN;

  // set up DMA1_CH7 (not used by anything else)
  RCC->AHB1ENR |= RCC_AHB1ENR_DMA1EN;                          // enable DMA
  DMA1_Channel8->CNDTR = transfers/2;                            // number of transfers
  DMA1_Channel8->CPAR = (uint32_t)&(GPIOB->ODR);               // write to register TODO: get from object
  DMA1_Channel8->CMAR = (uint32_t)&DD_data_buffer[1];             // read from address
  // set mem size to 32 to use every second value and peripheral size to 16 bit, increment mem address, direction to peripheral, set very high priority // enable transfer complete interrupt
  DMA1_Channel8->CCR |= (DMA_CCR_MSIZE_1 | DMA_CCR_PSIZE_0 | DMA_CCR_MINC | DMA_CCR_DIR | DMA_CCR_PL_0 | DMA_CCR_PL_1); // | DMA_CCR_TCIE);

  // enable DMAMUX
  RCC->AHB1ENR |= RCC_AHB1ENR_DMAMUX1EN;
  // set up DMAMUX, DMA1_CH8 is linked to DMAMUX1_CH7, TIM8_CH1 is on MUX 49
  DMAMUX1_Channel7->CCR |= (STM32_DMAMUX1_TIM1_CH2); //  && DMAMUX_CxCR_DMAREQ_ID_Msk);
...
// timer 1 generates the interleaved DMA trigger signals, clocked by TIM8
  RCC->APB2ENR |= RCC_APB2ENR_TIM1EN;                                           // enable clock to the timer system
  TIM1->ARR = 1;                                                          // configure overflow to set frequency
  TIM1->CCR1 = 0;                                                               // configure compare
  TIM1->CCR2 = 1;                                                             // configure compare
  TIM1->DIER |= (TIM_DIER_CC1DE | TIM_DIER_CC2DE);                              // enable CC match DMA request
  TIM1->CCER |= (TIM_CCER_CC1E | TIM_CCER_CC2E);                                // enable compare channels
  TIM1->CCMR1 |= (STM32_TIM_CCMR1_OC1M(6) | STM32_TIM_CCMR1_OC2M(6));             // PWM1 (positive) mode
  //TIM1->BDTR |= TIM_BDTR_MOE;                                                 // set main output enable bit
  TIM1->SMCR |= (STM32_TIM_SMCR_SMS(7) | STM32_TIM_SMCR_TS(5));                            // external clock mode, from internal trigger 5 (tim8_trgo)
  ...
  //TIM8->CR2 |= STM32_TIM_CR2_MMS(4);                                            // master mode selection: OC1 used as trigger out (use update: 2)
...
void DD_timer_DMA_stop(void)
{
  ...
  TIM1->CR1 = 0;            // deactivate timer
  DMA1_Channel8->CCR = 0;   // reset DMA
...
void cmd_DD_test(BaseSequentialStream *chp, int argc, char *argv[])
{
  ...
  double first_voltage = 0, second_voltage = 0, time = -2;
  DD_mode_t mode = NO_MODE;

  if(argc != 4)
  {
    chprintf(chp, "Usage:DD_test M Xs YV ZV\r\n");
    chprintf(chp, "Where M is the mode, RAMP (up OR down) or DOUBLE_RAMP (up AND down).\r\n");
    chprintf(chp, "X is the desired duration of each ramp.\r\n");
    chprintf(chp, "Y is the initial voltage and Z the target/second voltage.\r\n");
    chprintf(chp, "You may enter times and voltages as floats, time also in us.\r\n");
    return;
  }

  // process first argument
  if (!strcasecmp(argv[0], "RAMP"))
    mode = RAMP;
  else if (!strcasecmp(argv[0], "DOUBLE_RAMP"))
    mode = DOUBLE_RAMP;
  else
  {
  ...
  {
    chprintf(chp, "Error: Time is not valid.\r\n");
    return;
  }
  ...
  if (time <= 0)
  {
    chprintf(chp, "Error: Time has to be larger than zero.\r\n");
    return;
  }
  ...
  calculate_parameters(&time, &bits, &ARR, &f_timer);
  ...
  const ShellConfig current_shell_cfg = {chp, NULL};
...
static THD_FUNCTION(Thread_DD, arg)
  ...
  while (TRUE)
  {
    ...
    // fill data buffer
    // calculate assuming full range
    DD_data_buffer[0] = 0;
    if (DD_running_test.mode == RAMP)
    {
      transfers = exp2(DD_running_test.bits);
      for (int i = 1; i < transfers; i++)
        DD_data_buffer[i] = DD_data_buffer[i-1] + exp2(BIT_DEPTH - DD_running_test.bits);
    }
    else if (DD_running_test.mode == DOUBLE_RAMP)
    {
      transfers = 2 * exp2(DD_running_test.bits);
      for (int i = 1; i < transfers/2; i++)
              DD_data_buffer[i] = DD_data_buffer[i-1] + exp2(BIT_DEPTH - DD_running_test.bits);
      for (int i = transfers/2; i < transfers; i++)
              DD_data_buffer[i] = DD_data_buffer[i-1] - exp2(BIT_DEPTH - DD_running_test.bits);
    }
    ...
    // set ODR to start value already, generate single rising flank
    GPIOB->ODR = 0; //DD_data_buffer[0];
    __NOP();
    __NOP();
    ...
    // set ODR to middle of range (0 V), generate single rising flank
